...
A request's parameters (either from a query string or from POSTed form data) can be
documented using `requestParameters`. For example:

[source,java,indent=0]
----
include::{examples-dir}/com/example/RequestParameters.java[tags=request-parameters]
----
...
The result is a snippet named `request-parameters.adoc` that contains a table describing
the parameters that are supported by the resource.
...
TIP: If you are using MockMvc, to make the path parameters available for documentation,
...
A similar approach is also possible with REST Assured:
...
With this configuration in place, every call to the service made via Spring MVC Test or
REST Assured will produce the <<documenting-your-api-default-snippets,default snippets>>
without any further configuration. Take a look at the `GettingStartedDocumentation`
classes in each of the sample applications to see this functionality in action.
