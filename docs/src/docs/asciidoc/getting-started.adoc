...
----
<1> Add a dependency on `spring-restdocs-mockmvc` in the `test` scope. If you want to
	use REST Assured rather than MockMvc, add a dependency on
	`spring-restdocs-restassured` instead.
...
[[getting-started-documentation-snippets-common-setup]]
==== Common setup
...
With the JUnit rule in place, the steps now vary depending on whether you're using
Spring MVC Test or REST Assured.


[[getting-started-documentation-snippets-mockmvc]]
==== Generating documentation snippets with Spring MVC Test



[[getting-started-documentation-snippets-mockmvc-setup]]
===== Setting up Spring MVC test

To set up MVC Test, provide an `@Before` method that creates a `MockMvc` instance:

[source,java,indent=0]
----
include::{examples-dir}/com/example/mockmvc/ExampleApplicationTests.java[tags=mock-mvc-setup]
----
...
[[getting-started-documentation-snippets-mockmvc-invoking-the-service]]
...
[[getting-started-documentation-snippets-rest-assured]]
==== Generating documentation snippets with Rest Assured



[[getting-started-documentation-snippets-rest-assured-setup]]
===== Setting up REST Assured

To set up REST Assured, provide an `@Before` method that builds a `RequestSpecification`:

[source,java,indent=0]
----
include::{examples-dir}/com/example/restassured/ExampleApplicationTests.java[tags=rest-assured-setup]
----

The `RequestSpecification` is built with a `RestDocumentationFilter` which configures and
sets up Spring REST Docs within REST Assured. An instance of this class can be obtained
from the static `documentationConfiguration()` method on
`org.springframework.restdocs.restassured.RestAssuredRestDocumentation`.
`RestDocumentationFilter` applies sensible defaults and also provides an API for
customizing the configuration. Refer to the <<configuration, configuration section>> for
more information.
...
Now that a `RequestSpecification` instance has been created, it can be used when invoking
the RESTful service. For example:

[source,java,indent=0]
----
include::{examples-dir}/com/example/restassured/InvokeService.java[tags=invoke-service]
----
<1> Apply the `RequestSpecification` that configures Spring REST Docs within REST Assured.
<2> Set the `Accept` header to indicate that an `application/json` response is required.
<3> Install a `Filter` that will document the call to the service, writing the snippets
	into a directory named `index` that will be located beneath the configured output
	directory. The snippets are written by a `RestDocumentationFilter`. An instance of
	this class can be obtained from the static `document` method on
	`org.springframework.restdocs.restassured.RestAssuredRestDocumentation`.
<4> Invoke the service by making a `GET` request to `/`. By default the request is sent
	to `http://localhost:8080`. This can be changed using REST Assured's `baseUri` method.
<5> Assert that the service produced the expected response.
